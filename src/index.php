<?php
include "./modules/header.php";
?>

<!------------------------------------------------------------->

<?php
include "./modules/main-top.php";
include "./modules/about-section.php";
include "./modules/actions-section.php";
include "./modules/fleet-primary-section.php";
include "./modules/video-section.php";
include "./modules/partners-section.php";
include "./modules/text-and-picture-section.php";
include "./modules/text-and-slider-section.php";
include "./modules/our-news-section.php";
include "./modules/features-section.php";
include "./modules/history-section.php";
include "./modules/faq-section.php";
?>


<!------------------------------------------------------------->

<?php
include "./modules/footer.php";
?>

